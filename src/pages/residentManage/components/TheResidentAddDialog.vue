<template>
  <div class="dialogBoxClass"
    ...
    <div class="dialogHeaderClass">
      <el-row type="flex"  justify="space-between">
        ...
        <el-col class="header_right_box" :span="20">
          <el-button v-if="titleTxt === '新增居民'" :loading="isloading" @click="confirm(false)"  type="primary">保存并新增</el-button>
          <el-button type="primary" :loading="isloading" @click="confirm(true)" >确认</el-button>
      ...
      <div class="body_box">
        ...
        <el-row type="flex"
          ...
          <el-col :span="17">
            <tag-view style="display:inline;margin-left: -14px;"
                      :tags="dynamicTags"
                      @handleClose="handleClose($event,dynamicTags)"></tag-view>
        ...
        <el-row type="flex"
          ...
          <el-col :span="13">
            <tag-view style="display:inline;margin-left: -14px;"
                      :tags="featureList"
                      @handleClose="handleClose($event,featureList)"></tag-view>
            <p style="display: inline-block">
              <img style="vertical-align:middle;margin-left:10px;margin-right:7px;"
                   src="@/assets/images/personMange/addCard.png"
                   alt
        ...
        <el-row type="flex"
          ...
          <el-col :span="13">
            <tag-view style="display:inline;margin-left: -14px;"
                      :tags="strengthList"
                      @handleClose="handleClose($event,strengthList)"></tag-view>
            <p style="display: inline-block">
              <img style="vertical-align:middle;margin-left:10px;margin-right:7px;"
                   src="@/assets/images/personMange/addCard.png" />
              <span class="themeFont cursorClass"
                    @click="PFTShowAct('实有力量类型')">修改</span>
            </p>
          </el-col>
          ...
          <el-col :span="17">
            ...
            <div class="card_icon_class">
              <p v-for="(item, index) in cardList"
                ...
                <span>
                  <span class="cursorClass" @click="editCardAct(index,item)"
                        style="color:#26D39D;padding:0 12px">编辑</span>
                  <span class="cursorClass" @click="deleteCardAct(index,item)"
    ...
    <person-feature-dialog :isShow.sync="PFTShow"
                           :title="PFTitle"
                           :initSelectData="initSelectData"
                           @transferAct="transferTagsAct" />
...
import personFeatureDialog from "@/common/baseComponent/PersonFeature.vue";
...
export default {
  ...
  mounted() {
    ...
    this.categoryOptions = this.$common.getEnumByGroupStr("category") || [];
    this.category = this.categoryOptions[0].typeStr;
  },
  methods: {
    blurAct() {
      let isValite = this.$common.isCredentialNo(this.credentialNo);
      if (this.certificateOption === "id_card" && !isValite) {
    ...
    PFTShowAct(is) {
      this.PFTitle = is;
      if (is === "实有力量类型") {
        this.initSelectData = this.strengthList;
      } else {
        this.initSelectData = this.featureList;
      }
      this.PFTShow = !this.PFTShow;
    },
    getIcdIdCard() {
      // 发信令，读取
      this.initWebsocket();
    ...
    transferTagsAct(is, arr) {
      let nums = arr.map(item => {
        item.icon = this.icons.peopleflow;
        return item;
      });
      if (is === "实有力量类型") {
        this.strengthList = nums;
      } else {
        this.featureList = nums;
      }
    },
    handleClose($event, arr) {
      arr = $event;
    ...
    confirm(isBool) {
      ...
      var xhr = {
        ...
        strengthList: this.strengthList.map(item => item.id),
        featureList: this.featureList.map(item => item.id),
      ...
      this.isloading = true;
      this.transferResidentData(xhr, isBool);
    },
    transferResidentData(xhr, isBool) {
      console.log("添加居民信息", xhr);
      if (this.defaultResident.staffUuid) {
        this.$ResidentManageAjax
          .putResidentApi(xhr)
          .then(res => {
    ...
    clearView() {
      ...
      this.strengthList = [];
      this.featureList = [];
      this.featureList.push(window.config.ManFeatureArr[0]);
    },
  ...
  watch: {
    ...
    defaultResident: {
      handler(val, oldVal) {
        ...
        this.staffOptions = this.$common.getEnumByGroupStr("staff_t") || [];
        this.staffOption = this.staffOptions[0]
          ? this.staffOptions[0].typeStr
          : "";
        ...
        // 转化实有力量的数组
        if (val.strengthList) {
          this.strengthList = window.config.StrengthArr.filter(i => {
            i.icon = this.icons.peopleflow;
            return val.strengthList.some(item => {
              return i.id === item;
            });
          });
        }
        ...
        // 转化人员特征的数组
        if (val.featureList) {
          this.featureList = window.config.ManFeatureArr.filter(i => {
            i.icon = this.icons.peopleflow;
            return val.featureList.some(item => {
              return i.id === item;
            });
          });
        } else {
          let strengthObj = window.config.ManFeatureArr[0];
          strengthObj.icon = this.icons.peopleflow;
          this.featureList = [];
          this.featureList.push(strengthObj);
        }
        this.categoryOptions = this.$common.getEnumByGroupStr("category") || [];
        this.category = val.category || this.categoryOptions[0].typeStr;
      },
      deep: true,
      immediate: true
    },
