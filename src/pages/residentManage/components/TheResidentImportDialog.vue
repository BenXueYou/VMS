	...
	>
		...
		<div class="dialog-content">
			<div class="content-intro">
				...
				<div class="intro-download" @click="onClickDownload">
					<img src="@/assets/images/download_template.png" />
					<div class="download-text">下载人员模板</div>
				</div>
			</div>
			<div class="file-import" v-if="!startProgress">
				<div class="import-text">导入文件：</div>
				<!-- @click="onClickImportFile" -->
				<div class="import-button" type="primary" size="small" id="importBtnId">
			...
			<div class="file-import">
				...
				>{{fileName || "请选择上传文件"}}</div>
			</div>
			<div class="file-import" v-if="startProgress">
				<div class="import-text">导入进度：</div>
				<div class="import-text" style="width:150px;">
					<el-progress :percentage="percentage" color="#26D39D"></el-progress>
				</div>
			</div>

			<div class="file-import" v-if="percentage===100">
				<div class="import-text">导入结果：</div>
				<div
					class="import-text"
					style="width:300px;text-align:left;text-indent:30px;"
				>成功{{successPeople}}人 失败{{failPeople}}人</div>
				<div
					class="important-text download-text"
					v-if="failPeople>0"
					@click="downloadLog"
		...
		<div slot="footer" class="dialog-footer">
			<el-button type="primary" :loading="isImporting" @click="onClickImport">导入</el-button>
			<el-button type="primary" @click="onClickCancel">取消</el-button>
		</div>
	</el-dialog>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    getResult() {
      ...
      api
        .importResult({ importTaksUuid: this.importTaksUuid })
        .then(res => {
          console.log(res.data);
          this.isImporting = false;
          if (res.data.success) {
			...
			this.uploader = new plupload.Uploader({
				...
				init: {
					FileFiltered(up, file) {
						...
						console.log("1111111111111111111", file);
					},
					FilesAdded: function(up, files) {
						console.log("2222222222222222222222");
						console.log(up, files);
						if (this.startProgress) {
							this.$message.warning("正在导入，请稍候");
							return;
						}
						...
						plupload.each(files, function(file) {
							file.percent = 0;
							//这里在外面使用个全局的数组来记录上传了多少个文件然后进行处理操作。
							_this.fileIdNum.push(file);
					...
					UploadProgress: function(up, file) {
						console.log("3333333333333333333333", file);
					},
					/**
					 * 队列某个文件上传完成进行的回调
					...
					FileUploaded(uploader, file, responseObject) {
						console.log("444444444444444444444444", file);
					...
					UploadComplete(uploader, files) {
						this.isImporting = false;
						console.log(this.isImporting, "5555555555555555555555", files);
						for (var i = 0; i < _this.fileIdNum.length; i++) {
							_this.uploader.removeFile(_this.fileIdNum[i].id);
						}
						// _this.fileIdNum = [];
