	...
	>
		<div class="con">
			...
			<div class="btnWrap">
				...
				<el-button style="width: 66px;" @click="closeDialog">取消</el-button>
			</div>
		</div>
	</el-dialog>
</template>
  ...
  methods: {
    ...
    addCardConfirm() {
      ...
      for (let i = 0; i < this.cardList.length; i++) {
        if (this.checkedCard.cardId && i === this.currentCardIndex) {
          continue;
        } else {
          if (
            this.addCardName === this.cardList[i].cardName &&
						this.addCardFunc === this.cardList[i].function
          ) {
