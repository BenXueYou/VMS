<template>
	<div
		class="wrap theResidentGroup"
		v-loading="mainScreenLoading"
		element-loading-background="rgba(0, 0, 0, 0.8)"
	>
		<div class="contentBox" v-show="!addResidentDialogVisible">
			...
			<div class="tablecontent">
				<div class="overflowX">
					...
					>
						<el-table-column type="selection" :index="tableIndex" width="55"></el-table-column>
						...
						<el-table-column prop="staffType" width="100" label="人员类型">
							<template slot-scope="scope">{{$common.getEnumItemName("staff_t", scope.row.staffType)}}</template>
						</el-table-column>
						<el-table-column prop="netStatus" label="凭证">
							<template slot-scope="scope">
								<div class="tableCertificateClass">
									<span class="tableCertificateBtnClass">
										<img
											v-if="scope.row && scope.row.voucher && scope.row.voucher.faceNum"
											src="@/assets/images/personMange/face.png"
											style="margin-right: 11.5px"
										/>
										<img v-else src="@/assets/images/personMange/faceFade.png" style="margin-right: 11.5px" />
									</span>
									...
									<span class="tableCertificateBtnClass">
										<img v-if="true" src="@/assets/images/personMange/card.png" style="margin-right: 11.5px" />
										{{(scope.row && scope.row.voucher)?scope.row.voucher.cardNum:''}}
									</span>
								</div>
							</template>
						</el-table-column>
						<el-table-column prop="createTime" label="注册时间" :show-overflow-tooltip="true"></el-table-column>
		...
		<!----------------------------------居民添加弹窗-------------------------------------->
		<the-resident-add-dialog
			:addResidentDialogVisible="addResidentDialogVisible"
			:defaultResident="residentDetail"
			:titleTxt="TitleTxt"
			:topCheckedNodeTitle="topCheckedNodeTitle"
			@close="closeAddView"
			@transferResidentData="transferResidentData"
		></the-resident-add-dialog>
		...
		<the-resident-import-dialog
			:isShow="isShowPatchImportDialog"
			type="resident"
			title="批量导入居民"
		...
		<the-resident-detail-dialog
			:visible.sync="detailResidentDialogVisible"
			:defaultResident="residentDetail"
		></the-resident-detail-dialog>

		<!---------------------------------- 居民统计弹窗 TheResidentStaticsDialog---------------------------------->
		<statics-dialog
			:isShow="staticsResidentDialogVisible"
			:title="topCheckedNodeTitle"
			:staticDefaultData="staticDialogData"
			@onClickConfirm="staticsConfirm"
			@onClickCancel="staticsCancel"
		></statics-dialog>
	</div>
</template>
<script>
