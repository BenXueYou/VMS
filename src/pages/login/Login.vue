<template>
  <div class="login-div">
    <div class="login-head">
      <div style="width: 10%">
        <img src="@/assets/images/login/logo_big.png"
             width="100%" />
      </div>
      <div class="head-title">
        <span class="title"
              @click="hrefToCloud">后台管理系统登录</span>
        <img src="@/assets/images/login/change.png"
             width="20px"
             class="change-but"
             @click="hrefToCloud" />
      </div>
        ...
        <div class="login-buttons">
          ...
          <div style="margin-right: 4px">
            <span class="button-text"
                  style="cursor: pointer">忘记密码</span>
            <span class="button-text">|</span>
            <span class="button-text"
                  style="cursor: pointer">注册</span>
          </div>
        </div>
        <el-button style="width: 100%; margin-top: 48px;"
                   type="primary"
                   @click="login"
    ...
    <div class="login-foot">
      <div class="foot-item">
        <div class="item-text"
             v-for="(item, index) in alinkArr"
             :key="index">
          <a :href="item.strValue"
             target="_blank">{{ item.strName }}</a>
        </div>
        <!-- <div >关于广拓</div>
      ...
      <div style="margin:0px auto;font-size:14px;text-algin:center;padding:20px 0 10px">
        <p style="height:0px;line-height:20px;margin: 0px 0px 0px 5px; color:#939393;text-algin:center">
          2019© 上海广拓 沪ICP备09001829号-3
        </p>
      </div>
      <div style="margin:0 auto; padding:20px 0;">
        <a target="_blank"
          ...
          <p style="height:0px;line-height:20px;margin: 0px 0px 0px 5px; color:#939393; font-size:14px">
            沪公网安备 31010602003853号
          </p>
        </a>
      </div>
    </div>
...
export default {
  ...
  mounted() {
    this.formLabelAlign.username = this.$store.state.home.accountName;
    ...
    async loginSuccessResponse(body) {
      if (body) {
        ...
        if (projects.length > 1) {
          ...
          if (!body.data.adminUser.projects) {
          ...
          this.$store.dispatch(
            "setProjectType",
            body.data.adminUser.projects[0]
          );
          this.$store.dispatch(
            "setPlatformLevel",
            Number(
              Boolean(
                body.data.adminUser.projects[0].platformLevel === "levelOne"
              )
            )
          );
          this.$loginAjax
            ...
            .then(res => {});
          await this.getMapSet(body.data.adminUser.projects[0].projectUuid);
          this.$nextTick(() => {
