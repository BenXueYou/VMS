<template>
  <div class="FirstCardOpen">
    ...
    <!-- --------------------------    弹窗   ------------------------------------- -->
    <FirstCardOpen-dialog :isShow="isShow"
                          :FCDetail="FCDetail"
                          @onCancel="addFirstCardOpenAct"
                          @onConfirm="addFirstCardObj"></FirstCardOpen-dialog>
  </div>
</template>
<script>
    ...
    initData() {
      ...
      this.$DoorSetAjax.getFirstCardApi(xhr).then(res => {
        ...
        if (res.data.success && res.data.data) {
          ...
          this.tableData = list;
          this.total = res.data.data.total == null ? 0 : res.data.data.total;
        } else {
          this.$message({
            message: "没有找到相关数据",
            type: "warning"
