  ...
  methods: {
    ...
    sureClick() {
      ...
      for (let i = 0; i < this.timeOptions.length; i++) {
        if (this.data.time === this.timeOptions[i].value) {
          findIndex = i;
          break;
        }
      }
      if (findIndex === -1 && this.data.rule !== "all") {
        this.$message.error("");
        return;
      }
      for (let i = 0; i < this.dateOptions.length; i++) {
        if (this.data.date === this.dateOptions[i].value) {
          findIndex = i;
          break;
        }
      }
      if (findIndex === -1 && this.data.rule !== "all") {
        this.$message.error("请选择通行时间段或者特殊日期！");
        return;
      }
