		...
		<div class="dialog-content">
			...
			<div class="lock-info">
				<div class="info-pic">
					<template v-for="(item, index) in itemData.staffInfoGroup.staffGroupList">
						<div :key="index" class="img-block" @click="onClickPersonInfo(item)">
							<img :src="$common.setPictureShow(item.openDoorUrl)" width="100%" height="100%" />
						</div>
					</template>
				</div>
			...
			<div class="person-info">
				<div class="info-details">
					...
					>人员组名称：{{$common.setStringText(itemData.staffInfoGroup.staffGroupName)}}</div>
    ...
    isShow(val) {
      ...
      if (val) {
        for (let index in this.itemData.staffInfoGroup.staffGroupList) {
          if (parseInt(index) === 0) {
            let staffGroupList = this.itemData.staffInfoGroup.staffGroupList;
            this.groupStaffNames = staffGroupList[index].staffName;
          } else {
