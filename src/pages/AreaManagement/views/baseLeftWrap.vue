<template>
  <div id="equipLeftMenu"
    ...
    <div class="searchWrap">
      ...
      <img @click="showAddChildrenDialog" :src="icons.add" alt />
    </div>
    <div class="tabs">
      <div class="mypenl">
        <gt-tree ref="tree1"
...
export default {
  ...
  methods: {
      ...
      api
        ...
        .then(res => {
          if (res.data.data) {
            ...
            for (let i = 0; i < result.length; i++) {
              result[i].orgUuid = result[i].id;
              result[i].selected = false;
            }
            if (result.length) {
              result[0].isextend = true;
              result[0].havechildren = !!result[0].children.length;
            }
            this.data = result;
            console.log(this.data);
            // if (isFirst && result && result.length) {
            //   // 如果是第一次进来，自动选第一个子级并且展开。则自动展开子级。
            //   this.parentOrgUuid = result[0].id;
            //   if (this.orgType === "device") {
            //     this.$store.commit("setOrgUuid", result[0].id);
            //   }
            //   this.Treeparent = "0-0";
            //   this.getOrgTree();
            //   // 自动选中第一行有问题
    ...
    getChidrendata(orgUuid, isextend, parent) {
      console.log(orgUuid, isextend, parent);
