        ...
        .then(res => {
          ...
          if (res.data.success) {
            ...
            for (let i = 0, len = list.length; i < len; i++) {
              ...
              let picture = [];
              picture.push(listObj);
              if (i > 0 && list[i].time === list[i - 1].time) {
                this.data[this.data.length - 1].picture.unshift(listObj);
              } else {
                listObj.picture = picture;
                this.data.push(listObj);
              }
            }
            this.data.sort((a, b) => {
            ...
            for (let j = 0; j < this.data.length; j++) {
              ...
              for (let k = 0; k < picture.length; k++) {
                ...
                let index = dateChannelArr.findIndex(item => {
                  return item.channelName === picture[k].channelName;
                });
                // 创建通道下的抓拍列表
