<template>
	<div class="content" v-loading="mainListLoading1" element-loading-background="rgba(0, 0, 0, 0.8)">
		<!-- <keep-alive>
				...
				<div class="left">
					<el-popover placement="top-start" title="标题" width="200" trigger="click">
						...
						<div class="selectMask" slot="reference">
							<el-select v-model="unitChoose" placeholder="请选择单元" style="width: 120px;position: relative;"></el-select>
					...
					<el-popover placement="top-start" title="标题" width="200" trigger="click">
						...
						<div class="selectMask" slot="reference" style="margin: 0 46px 0 20px">
							<el-select v-model="roomChoose" placeholder="请选择室" style="width: 120px;"></el-select>
							<div class="maskCon"></div>
						</div>
					</el-popover>
					<div class="marRight">总人口：{{totalPeopleNumber}}人</div>
					<div>重点关注人员： {{totalFocuseNumber}}人</div>
					<el-popover placement="top-start" class="popverTree" title="标题" width="200" trigger="hover"></el-popover>
				</div>
				<div class="right">
					<el-button
						type="primary"
						...
						<div class="listTextWrap">
							<div class="singleText">
								...
								<span v-if="tableData.length>index">
									...
									<span
										v-if="tableData[tableData.length-index-1].staffType"
									>{{$common.transferEnumItemName($store.getters.GET_STAFFOPTIONS, tableData[tableData.length-index-1].staffType)}}</span>
								</span>
							</div>
							<div
								class="singleText"
							>{{tableData.length>index?tableData[tableData.length-index-1].address:"住址"}}</div>
						</div>
					</div>
				</div>
			</div>
