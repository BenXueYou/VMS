	...
	<div class="dialogcontainer">
		<el-dialog :visible.sync="dialogIsShow" @open="open" title="研判详情" @close="close" @closed="closed">
			<div class="dialogwrap">
				...
				<div class="form-box">
					<div class="left">
						<p class="label">处理意见：</p>
					</div>
					<div class="right">
						<el-input type="textarea" rows="3" v-model="dealsuggestion"></el-input>

						<el-button class="comfirm" type="primary" @click="comfirmbtn">确认</el-button>

						<el-button class="ignore" type="primary" @click="ignorebtn">忽略</el-button>
					</div>
				</div>
			</div>
		</el-dialog>
  ...
  methods: {
    ...
    updateStatus() {
      ...
      for (let i = 0, len = this.data.length; i < len; i++) {
        ...
        if (!ob[this.data[i].devicesname]) {
          ob[this.data[i].devicesname] = devicecolor[cnt % devicecolor.length];
          cnt++;
          this.data[i].color = ob[this.data[i].devicesname];
          arr.push({
            name: this.data[i].devicesname,
            color: this.data[i].color
          });
          slectedarr.push(this.data[i].devicesname);
        } else {
          this.data[i].color = ob[this.data[i].devicesname];
        }
      }
    ...
    comfirmbtn() {
      ...
      if (
        !this.row.vconfirmuuid ||
				!this.row.libraryUuid ||
				!this.row.photouri ||
				!this.row.photoUuid ||
				!this.row.staffUuid
      ) {
        this.$message.info("该条数据缺少信息，");
        return;
      }
      var obj = {
        alarminfoid: this.row.vconfirmuuid,
        libraryuuid: this.row.libraryUuid,
        photouri: this.row.photouri,
        photouuid: this.row.photoUuid,
        staffuuid: this.row.staffUuid,
        suggestion: this.dealsuggestion
      };
    ...
    ignorebtn() {
      if (
        !this.row.vdetecteduuid ||
				!this.row.libraryUuid ||
				!this.row.photouri ||
				!this.row.photoUuid ||
				!this.row.staffUuid
      ) {
        this.$message.error("缺少数据,不可以提交！");
        return;
      }
      var obj = {
        alarminfoid: this.row.vdetecteduuid,
        libraryuuid: this.row.libraryUuid,
        photouri: this.row.photouri,
        photouuid: this.row.photoUuid,
        staffuuid: this.row.staffUuid,
