	...
	<div class="main-block">
		...
		<div class="main-container">
			<div class="search">
				...
				<el-input v-model="modelName" style="width: 160px;"></el-input>
				<span style="margin-left: 4%">状态：</span>
				<el-select style="width: 160px;" v-model="status" clearable size="small">
					<el-option
					...
					<template v-for="(item, index) in moduleList">
						<div :key="index" class="list-item" @click="lookDetail(item)">
							...
							<div class="info-other">
								...
								<div class="other-span desc">
									{{item.staffName}}
									<span
										:style="item.staffName ? 'margin-left: 5px;' : ''"
									>{{item.faceLibraryName}}</span>
								</div>
								<div
									class="other-span"
									:style="item.status === 'model_processed' ? 'color:#26D39D' : 'color:#FD545E'"
								>{{$common.getEnumItemName('model_analysis_s', item.status)}}</div>
							</div>
						</div>
					</template>
				</div>
				<div class="list-table" v-loading="isLoading" v-else>
					<el-table :data="moduleList" style="width: 100%">
						<el-table-column type="index" label="序号" width="60"></el-table-column>
						<el-table-column prop="faceModelName" label="模型名称" show-overflow-tooltip></el-table-column>
						<el-table-column prop="createTime" label="研判时间" show-overflow-tooltip></el-table-column>
						<el-table-column prop="analysisResultDescribe" label="研判描述" show-overflow-tooltip></el-table-column>
						<el-table-column prop="faceLibraryName" label="所属库" show-overflow-tooltip></el-table-column>
						<el-table-column prop="status" label="状态">
							<template
								slot-scope="scope"
							>{{$common.getEnumItemName('model_analysis_s', scope.row.status)}}</template>
						</el-table-column>
