		...
		<div class="details" :style="`width: ${width}`" v-show="isShowMain">
			...
			<div class="button">
				<div class="button-right">
					<el-switch
						v-model="faceMonitorObj.enabled"
						active-color="rgba(32,204,150,0.2)"
						inactive-color="rgba(255,255,255,0.2)"
						@change="switchChange"
						:disabled="!OwnAuthDisabled || (faceMonitorObj.faceMonitorUuid === 'temp_face_monitoruuid00020191104') ? true : false"
						:active-value="1"
						:inactive-value="0"
					></el-switch>
					...
					>
						...
						<span :disabled="!OwnAuthDisabled" style="margin-left: 3px;">删除</span>
					</div>
				</div>
			</div>
			<div class="container">
				<div class="info-container" style="margin: 18px 0;">
					<div class="info-block block-line" style="width: 23%">名称：{{faceMonitorObj.faceMonitorName}}</div>
					<div class="info-block block-line" style="width: 23%">创建人：{{faceMonitorObj.createUserName}}</div>
					<div class="info-block block-line" style="width: 23%">创建时间：{{faceMonitorObj.createTime}}</div>
					<div class="info-block block-line" style="width: 23%; background: transparent;"></div>
				</div>
				<source-show-more
					:title="`人脸库： 共${faceMonitorObj.libraryList ? faceMonitorObj.libraryList.length : 0}个人脸库`"
					blockHeight="80px"
				...
				/>
				<source-show-more
					:title="`视频源： 共${faceMonitorObj.channelList ? faceMonitorObj.channelList.length : 0}个摄像头`"
					blockHeight="120px"
				...
				<div class="info-container" style="margin: 18px 0;">
					<div
						...
						style="width: 10%"
					>阈值：{{`${faceMonitorObj.faceSimilarityThreshold}%`}}</div>
					<div
						class="info-block block-line"
						style="width: 11%"
					>是否报警：{{faceMonitorObj.alarmed ? '是' : '否'}}</div>
					<div
						class="info-block block-line"
						style="width: 11%"
					>是否报警弹窗：{{faceMonitorObj.popup ? '是' : '否'}}</div>
					<div class="info-block block-line" style="width: 13%">报警声音：{{faceMonitorObj.alarmSoundName}}</div>
					<div class="info-block block-line" style="width: 13%; display: flex; align-items: center">
						<span>布控颜色：</span>
						<div
							class="monitor-color"
							:style="`background-color:${faceMonitorObj.taskColour ? faceMonitorObj.taskColour : 'transparent'};`"
						></div>
					</div>
					<div class="info-block block-line" style="width: 13%">目标人列表：{{faceMonitorObj.reservedCount}}</div>
					<div
						class="info-block block-line"
						style="width: 20%"
					>人脸抓拍图片需达到的质量：{{faceMonitorObj.faceCapturePhotoQualities ? $common.getEnumItemName("face_quality", faceMonitorObj.faceCapturePhotoQualities[faceMonitorObj.faceCapturePhotoQualities.length-1]) : ""}}</div>
				</div>
				<div class="list-title">
					...
					<div class="title-right">
						...
						<el-button :disabled="!ShowAuthDisabled" @click="turnToAlarm" type="primary" size="small">
							<div style="display: flex; align-items: center;">
