<template>
	<div class="personflow">
		<div class="select-group">
			<img src="@/assets/images/sort.png" width="15.1px" height="12px" style="margin:0 0.8% 0 0;" />
			<div class="title-text">排序：</div>
			<el-radio-group v-model="sortRadio" @change="handleSortChange" style="margin: 2px 0 0 0.5%">
			...
			<img
				...
				width="15.1px"
				height="12px"
				style="margin:0 0.8% 0 5%;"
			/>
			<div class="title-text">报表类型：</div>
			<el-radio-group v-model="typeRadio" @change="handleTypeChange" style="margin: 4px 0 0 0.5%;">
...
export default {
  ...
  methods: {
    ...
    searchData() {
      if (this.typeRadio === 1) {
        this.getFaceCompareSumByDay();
      } else if (this.typeRadio === 2) {
        this.getFaceCompareSumByMonth();
      }
    },
    resetData() {
      this.channelName = [];
    ...
    getFaceCompareSumByDay() {
      ...
      date = date + ":00:00";
      this.$statisticRequest.getFaceCompareSumByDay({
        sort: this.sortRadio,
        date: `${this.dateValue} ${date}`,
        onResult: (isSuccess, data) => {
          if (isSuccess && data) {
            this.handleGetFaceCompareSumByDaySuccessResponse(data);
          } else {
            console.log("获取人脸比对(日)统计失败！");
          }
        }
      });
    },
    handleGetFaceCompareSumByDaySuccessResponse(data) {
      this.resetData();
      if (data.body.data) {
    ...
    getFaceCompareSumByMonth() {
      let date = this.changeNum(this.timeValue);
      this.$statisticRequest.getFaceCompareSumByMonth({
        sort: this.sortRadio,
        date: `${this.dateValue}-${date}`,
        onResult: (isSuccess, data) => {
          if (isSuccess && data) {
