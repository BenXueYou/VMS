          ...
          <div class="same-place-inner">
            <template v-for="(item, index) in samePlaArr">
              <div :key="index"
                ...
                <div class="tipsIconClass">{{index + 1}}</div>
              </div>
            </template>
          </div>
          <div class="close"
...
export default {
  ...
  methods: {
    ...
    getSamePlaceArr(itemData) {
      this.samePlaArr = [];
      this.getTragicItemList(itemData);
    },
    closeSamePlaDialog() {
    ...
    initData() {
      ...
      let initPoint = new BMap.Point(121.481115, 31.23667);
    ...
    getTragicItemList(item) {
      this.$factTragicHttp
        ...
        .then(res => {
          let body = res.data;
          this.getTragicItemListSuccess(body);
        });
    },
    getTragicItemListSuccess(body) {
      this.samePlaArr = body.data;
    },
    setMapCenter() {
      if (this.itemData.length !== 0) {
        this.map.setCenter(
