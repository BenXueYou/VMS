<template>
	<div class="main-block">
		<look-all-view :isShow="isShowAllView" @onCancel="onCancelAllView" :item="selectedItem" />
			...
			<div class="search">
              ...
              :disabled='!OwnAuthDisabled'
				...
				<div class="search-select">
					...
					<el-radio-group v-model="libraryType" @change="handleTypeChange" style="margin: 4px 0 0 0.5%;">
					...
					<el-input v-model="similarity" type="number" style="width: 55px;"></el-input>
					<span style="margin-left: 4px">%</span>
					<el-button :disabled="!ShowAuthDisabled" @click="search" type="primary" size="small" class="search-btn">搜索</el-button>
				</div>
			</div>
			<div class="result">
				...
				<div class="result-list" v-loading="isLoading">
					<template v-for="(item, index) in faceList">
						<div :key="index" class="list-item">
							...
							/>
							<img
								:src="$common.setPictureShow(item.faceCapturePhotoUrl)"
								height="100%"
								width="40%"
								v-else
								class="img-fill"
							/>
							<div class="item-info">
								<div class="info-name" v-if="libraryType !== 'captureFaceLib'">{{item.staffName}}</div>
								<div class="info-other">
									<div class="other-span">性别：{{$common.getEnumItemName("gender", item.genderCapture)}}</div>
									<div
										class="other-span text-show"
									>所属库：{{libraryType === 'captureFaceLib' ? '抓拍库' : item.faceLibraryName}}</div>
									<div
										class="other-span text-show"
										v-if="libraryType !== 'captureFaceLib'"
									>人员类型：{{$common.getEnumItemName('staff_t', item.staffType)}}</div>
							...
							<div class="item-menu">
								<div :disabled="!ShowAuthDisabled" class="menu-elem" @click="turnToSearchFace(item)">
									<img src="@/assets/images/faceModule/face_search.png" />
									<span class="elem-title">以脸搜脸</span>
								</div>
								<div :disabled="!ShowAuthDisabled"  class="menu-elem" @click="turnToPersonTrace(item)">
									<img src="@/assets/images/faceModule/person_route.png" />
									<span class="elem-title">人员轨迹</span>
								</div>
								<div :disabled="!ShowAuthDisabled"  class="menu-elem" v-if="libraryType === 'captureFaceLib'" @click="lookAllView(item)">
									<img src="@/assets/images/faceModule/look_allview.png" />
									<span class="elem-title">查看全景图</span>
								</div>
								<div :disabled="!OwnAuthDisabled"  class="menu-elem" v-if="libraryType === 'captureFaceLib'" @click="tempMonitor(item)">
									<img src="@/assets/images/faceModule/now_dect.png" />
									<span class="elem-title">临时布控</span>
								</div>
								<div :disabled="!OwnAuthDisabled"  class="menu-elem" @click="downloadImage(item)">
