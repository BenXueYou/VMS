    ...
    <div class="RecognizeCardClass">
      <el-row class="asidListRowHeader"
        ...
        <div class="rowHeaderBox"
             :style="`background-color:${recoginizeItem.taskColour?recoginizeItem.taskColour:'#26D39D'};`">
          <el-col :span="18"
      ...
      <div class="asidListRowBody">
        ...
        <div class="progressBox">
          <el-progress class="asidRowProgress"
                       ...
                       :percentage="recoginizeItem.similarity||recoginizeItem.faceSimilarity || 0"></el-progress>
          <div class="asidFontClass"
               style="visibility:hidden">布控照</div>
        </div>
        <div class="relativeClass">
          <div class="asidCompareImgBox"
               :style="`width:${imgWidth}px;height:${imgWidth}px`">
            <img class="asidCardImg"
                 :src="$common.setPictureShow(recoginizeItem.facePhotoUrl)" />
          </div>
      ...
      <div v-if="alarmState"
           class="alarmStateTxt">{{$common.getEnumItemName("alarm_r",alarmState)||'已处理'}}</div>
    </div>
  </transition>
</template>
<script>
