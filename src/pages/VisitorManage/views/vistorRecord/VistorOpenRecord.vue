<template>
  <div class="VistorOpenRecord"
    ...
    <div class="main">
      <div class="main-header">
        <div class="leftgroup">
          ...
          <el-date-picker v-model="validateTimeEnd"
                          ...
                          placeholder="选择日期"
                          value-format="yyyy-MM-dd HH:mm:ss"></el-date-picker>
          <el-button :disabled="!$common.getAuthIsOwn('访客开门记录', 'isShow')"
                     type="primary"
      ...
      <el-table ref="VistorOpenRecordTable"
                :data="tableData"
                tooltip-effect="dark"
                class="tableBoxClass"
        ...
        <el-table-column prop="reservationType"
                         label="访客类型">
          <template slot-scope="scope">{{$common.getEnumItemName("visitor_type", scope.row.reservationType)}}</template>
        </el-table-column>
        <el-table-column prop="recognitionMode"
                         label="验证类型">
          <template slot-scope="scope">{{$common.transferValidateType(scope.row.recognitionMode)}}</template>
        </el-table-column>
        <el-table-column prop="channelNickName"
                         label="门"></el-table-column>
        ...
        <el-table-column prop="verifyDatetime"
                         label="验证时间"></el-table-column>
        <el-table-column label="操作">
          <template slot-scope="scope">
            <div class="tableCertificateBtnClass">
              <span :class="$common.getAuthIsOwn('访客开门记录', 'isShow')?'cursorClass':'disabled'"
      ...
      <vistor-open-detail :visible.sync="isShow"
                          :openDoorDetail="openDoorDetail"
                          @close="closeDetail"></vistor-open-detail>
      <!----------------------------------表格分页器---------------------------------->
      <div class="footer">
