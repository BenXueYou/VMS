async function slot_createCanvas() {
  // 1. 创建canvas
  ...
  if (video != null) await video_mgr.play(video);
...
async function slot_createVideo() {
  ...
  let jSignal = {
    ...
    routeType: "location",
    srcUuid: "signal_channel_9AF73B050B6D406886C0178CBA132101"
  };
  let jMedia = {
    identity: "slave",
    param: {
    ...
    routeType: "location",
    srcUuid: "media_channel_9AF73B050B6D406886C0178CBA132101"
  };
  ...
  document.getElementById("p").innerHTML = video_mgr.m_videoList.length;
  document.getElementById("div").appendChild(videoTag);
  var t = {};
  t.canvas = videoTag;
  t.video = video;
  t.action = action;
...
async function slot_download() {
  ...
  if (video != null) video_mgr.play(video);
...
function onStreamBreak(video) {
  ...
  for (let i = 0; i < video_list.length; ++i) {
    ...
    if (tmp == video) {
      video_mgr.stop(tmp);
      if (
        video_list[i].action == "preview" ||
        video_list[i].action == "playback"
      ) {
        document.getElementById("div").removeChild(video_list[i].canvas);
